...
def index():
    ...
    if request.method == "POST":
        ...
        users_stocks = db.execute(
            "SELECT * FROM users_stocks WHERE user_id = ?", session["user_id"])

        stocks_value = 0

        # I CANNOT do just lookups = [{}] because in for-loop I can't assign anything to the ith dictionary because it doesn't exist yet, took me over an hour, goddamn! Didn't come up yesterday when I only had one dictionary in the list. Stackoverflow saved me.
        lookups = [None] * len(users_stocks)
        for i in range(len(users_stocks)):
            lookups[i] = lookup(users_stocks[i]['stock'])
            lookups[i]['shares'] = users_stocks[i]['shares']
            lookups[i]['total'] = lookup(users_stocks[i]['stock'])[
                'price'] * users_stocks[i]['shares']
...
def buy():
    ...
    if request.method == "POST":
        ...
        # Ensures shares field is filled and not negative
            ...
            # or int(request.form.get("shares")) < 0:  #actually better to make field in html non-negative by setting min=1
            return apology("missing shares", 400)

        # ensure numeric input (fractional or negative inputs are handled by html)
        elif request.form.get("shares").isdigit() == 0:
        ...
        """# ensures that 112.00 is a valid input (even though it is not integer)
        # elif (request.form.get("shares") % 1) != 0:
        #   return apology("invalid shares", 400)

        # HOW TO STOP FRACTIONAL BUT ALLOW 112.0??

        # Ok, wow, no idea wth the "valid purchase" in check50 means,
        # but having commented this out, now opening up check50 in the morning
        # it's all green...it's rly a mystery, I guess at night maybe my brain is just not that sharp? Rly strange."""

        # Need to be careful with this """---""" commenting. I did this inbetween the "elifs", and it interrupted the flow...
        ...
        db.execute("INSERT INTO transactions (user_id, stock, shares, datetime, price) VALUES (?, ?, ?, ?, ?)",
                   session["user_id"], symbol, shares, current_time, price)
        ...
        if not db.execute("SELECT * FROM users_stocks WHERE user_id = ? AND stock = ?", session["user_id"], symbol):
            db.execute("INSERT INTO users_stocks (user_id, stock, shares) VALUES (?, ?, ?)",
                       session["user_id"], symbol, shares)
        # else just update the new share count of the stock he owns in his portfolio
        else:
            shares_owned = db.execute(
                "SELECT shares FROM users_stocks WHERE user_id = ? AND stock = ?", session["user_id"], symbol)
            print(shares_owned)
            print(type(shares_owned))
            db.execute("UPDATE users_stocks SET shares = ? WHERE user_id = ? AND stock = ?",
                       shares_owned[0]['shares']+shares, session["user_id"], symbol)
...
def register():
    ...
    if request.method == "POST":
        ...
        # ah yes, after 38 minutes or so I solved this one too, seemingly
        ...
        # or render_template("login.html", 200) or redirect("/login")
        return redirect("/")
...
def sell():
    ...
    if request.method == "POST":
        ...
        # update share count of stock
        db.execute("UPDATE users_stocks SET shares = ? WHERE user_id = ? AND stock = ?",
                   shares_owned-shares, session["user_id"], symbol)

        # delete any share that you don't own anymore (if shares==0)
        db.execute(
            "DELETE FROM users_stocks WHERE user_id = ? AND shares = ?", session["user_id"], 0)
